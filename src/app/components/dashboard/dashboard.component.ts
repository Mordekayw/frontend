    ...
    this.reservaService.obtenerTodas().subscribe({
      next: (reservas) => {
        ...
        this.reservasCliente = reservas.filter(
          (r: any) => r.cliente?.usuario?.username === this.user?.sub
        );
        this.reservasActivasCliente = this.reservasCliente.filter(
          (r: any) => r.estadoReserva === 'ACTIVA'
        ).length;
        this.reservasFinalizadasCliente = this.reservasCliente.filter(
          (r: any) => r.estadoReserva === 'FINALIZADA'
        ).length;
      },
