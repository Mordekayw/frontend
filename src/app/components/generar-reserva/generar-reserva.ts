import { Component, OnInit, ChangeDetectorRef } from '@angular/core';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { Router, RouterModule, ActivatedRoute } from '@angular/router';
import { NavbarComponent } from '../navbar/navbar.component';
import { ReservaService } from '../../services/reserva.service';
import { HabitacionService } from '../../services/habitacion.service';
import { ClienteService } from '../../services/cliente.service';
import { Reserva } from '../../models/reserva.model';
import { Habitacion } from '../../models/habitacion.model';
import { Cliente } from '../../models/cliente.model';

@Component({
  selector: 'app-generar-reserva',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterModule, NavbarComponent],
  templateUrl: './generar-reserva.html',
  styleUrl: './generar-reserva.scss',
})
export class GenerarReserva implements OnInit {
  reserva: Reserva = {
    fechaInicio: '',
    fechaFin: '',
    horaEntrada: '14:00',
    horaSalida: '12:00',
    diasEstadia: 0,
    totalPagar: 0,
    estadoReserva: 'PENDIENTE',
  };

  habitacionesDisponibles: Habitacion[] = [];
  habitacionSeleccionadaId: number | null = null;
  cliente: Cliente | null = null;
  dniBuscar: string = '';

  // Propiedades de Mensaje (infoMessage añadido para corregir el error)
  errorMessage: string = '';
  successMessage: string = '';
  infoMessage: string = ''; // <-- CORRECCIÓN AÑADIDA

  diasEstadia: number = 0;
  totalPagar: number = 0;

  constructor(
    private reservaService: ReservaService,
    private habitacionService: HabitacionService,
    private clienteService: ClienteService,
    private router: Router,
    private route: ActivatedRoute,
    private cd: ChangeDetectorRef
  ) {}

  ngOnInit() {
    this.cargarHabitaciones();
    this.route.queryParams.subscribe((params) => {
      if (params['dni']) {
        this.dniBuscar = params['dni'];
        this.buscarCliente();
      }
      if (params['idCliente']) {
        this.cargarClientePorId(Number(params['idCliente']));
      }
    });
  }

  // Método para limpiar todos los mensajes
  limpiarMensajes(): void {
    this.successMessage = '';
    this.errorMessage = '';
    this.infoMessage = '';
  }

  cargarHabitaciones() {
    this.habitacionService.obtenerDisponibles().subscribe({
      next: (data) => {
        this.limpiarMensajes();
        this.habitacionesDisponibles = data;
        if (data.length === 0) {
          this.infoMessage = 'No hay habitaciones disponibles actualmente.';
        }
      },
      error: (err) => {
        this.limpiarMensajes();
        this.errorMessage = 'Error al cargar habitaciones disponibles.';
        console.error(err);
      },
    });
  }

  buscarCliente() {
    this.limpiarMensajes();
    if (!this.dniBuscar || !/^\d{8}$/.test(this.dniBuscar)) {
      this.errorMessage = 'El DNI debe contener exactamente 8 dígitos numéricos';
      this.cliente = null;
      return;
    }

    this.clienteService.buscarPorDni(this.dniBuscar).subscribe({
      next: (data) => {
        this.cliente = data;
        this.reserva.cliente = data;
        this.infoMessage = data ? '' : 'Cliente no encontrado. Por favor, regístrelo primero.';
        this.errorMessage = '';
      },
      error: (err) => {
        // Usamos infoMessage/errorMessage para indicar que no se encontró el cliente
        this.infoMessage = 'Cliente no encontrado. Por favor, regístrelo primero.';
        this.cliente = null;
        this.errorMessage = '';
      },
    });
  }

  cargarClientePorId(id: number) {
    // Si tienes un método para cargar cliente por ID, úsalo aquí.
    // this.clienteService.buscarPorId(id).subscribe({...});
  }

  calcularCosto() {
    if (!this.habitacionSeleccionadaId || !this.reserva.fechaInicio || !this.reserva.fechaFin) {
      this.totalPagar = 0;
      this.diasEstadia = 0;
      return;
    }

    this.reservaService
      .calcularCosto(this.habitacionSeleccionadaId, this.reserva.fechaInicio, this.reserva.fechaFin)
      .subscribe({
        next: (data) => {
          this.limpiarMensajes();
          if (data.error) {
            this.errorMessage = data.error;
            this.diasEstadia = 0;
            this.totalPagar = 0;
            this.reserva.diasEstadia = 0;
            this.reserva.totalPagar = 0;
          } else {
            this.diasEstadia = data.dias;
            this.totalPagar = data.total;
            this.reserva.diasEstadia = data.dias;
            this.reserva.totalPagar = data.total;
          }
        },
        error: (err) => {
          this.limpiarMensajes();
          this.errorMessage = 'Error al calcular el costo';
          this.diasEstadia = 0;
          this.totalPagar = 0;
          this.reserva.diasEstadia = 0;
          this.reserva.totalPagar = 0;
          console.error(err);
        },
      });
  }

  onHabitacionChange() {
    this.limpiarMensajes();
    const habitacion = this.habitacionesDisponibles.find(
      (h) => h.id === this.habitacionSeleccionadaId
    );
    if (habitacion) {
      this.reserva.habitacion = habitacion;
      this.calcularCosto();
    } else {
      this.reserva.habitacion = undefined;
    }
  }

  onFechaChange() {
    this.limpiarMensajes();
    if (this.reserva.fechaInicio && this.reserva.fechaFin) {
      this.calcularCosto();
    }
  }

  guardarReserva() {
    this.limpiarMensajes();

    // Validaciones
    if (!this.reserva.cliente) {
      this.errorMessage = 'Debe buscar y seleccionar un cliente primero';
      return;
    }
    if (!this.habitacionSeleccionadaId) {
      this.errorMessage = 'Debe seleccionar una habitación';
      return;
    }
    if (!this.reserva.fechaInicio || !this.reserva.fechaFin) {
      this.errorMessage = 'Debe completar las fechas de entrada y salida';
      return;
    }
    if (this.totalPagar <= 0 || this.diasEstadia <= 0) {
      this.errorMessage =
        'Las fechas o la habitación seleccionada son inválidas. Verifique el cálculo de costo.';
      return;
    }

    // Ensure habitacion is set
    const habitacion = this.habitacionesDisponibles.find(
      (h) => h.id === this.habitacionSeleccionadaId
    );
    if (habitacion) {
      this.reserva.habitacion = habitacion;
    }

    // Construir payload explícito
    const payload: any = {
      // Si cliente existe y tiene ID, enviamos solo el ID, sino el objeto completo (si es que la API lo permite)
      cliente:
        this.reserva.cliente && (this.reserva.cliente as Cliente).id
          ? { id: (this.reserva.cliente as Cliente).id }
          : this.reserva.cliente,
      habitacion: { id: this.habitacionSeleccionadaId },
      fechaInicio: this.reserva.fechaInicio, // formato YYYY-MM-DD
      fechaFin: this.reserva.fechaFin, // formato YYYY-MM-DD
      // Asegurar formato de hora con segundos (HH:mm:ss) para backend
      horaEntrada:
        this.reserva.horaEntrada && this.reserva.horaEntrada.length === 5
          ? this.reserva.horaEntrada + ':00'
          : this.reserva.horaEntrada,
      horaSalida:
        this.reserva.horaSalida && this.reserva.horaSalida.length === 5
          ? this.reserva.horaSalida + ':00'
          : this.reserva.horaSalida,
      diasEstadia: this.reserva.diasEstadia,
      totalPagar: this.reserva.totalPagar,
      estadoReserva: this.reserva.estadoReserva,
      opcionesServicios: (this.reserva as any).opcionesServicios || {}, // Usar 'any' si 'opcionesServicios' no está en el modelo Reserva
    };

    this.reservaService.crearReserva(payload).subscribe({
      next: (data: any) => {
        this.limpiarMensajes();
        this.successMessage = 'Reserva creada exitosamente.';
        // Pequeño delay para que el usuario vea el mensaje de éxito
        setTimeout(() => {
          this.router.navigate(['/reservas']);
        }, 800);
      },
      error: (err) => {
        this.limpiarMensajes();
        this.errorMessage =
          err.error?.error || 'Error al crear la reserva. Por favor, revise la consola.';
        console.error(err);
      },
    });
  }
}
